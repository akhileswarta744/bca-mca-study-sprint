  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-16">
      ...
      <div className="p-6 md:p-8 rounded-3xl bg-slate-900/80 border border-slate-800 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-2">
          ...
          <p className="text-sm text-slate-300 max-w-3xl leading-relaxed">
            ...
            Categorized into <strong className="text-cyan-300">CORE COMPUTER</strong> and{' '}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950/40 border border-indigo-500/30 shadow-lg space-y-3">
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-amber-950/30 border border-amber-500/30 shadow-lg space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
      ...
      <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl space-y-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 transition-colors"
          />
          {searchQuery && (
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredTopics.map((topic) => {
          ...
          return (
            <div
              key={topic.id}
              className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-slate-750 hover:bg-slate-850/80 transition-all flex flex-col justify-between group shadow-sm"
            >
              <div className="space-y-3">
                {/* Subject tag & Day Badge */}
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[11px] font-bold text-indigo-400 truncate max-w-[170px]">
                    {subject?.name}
                  </span>
                ...
                <div className="flex flex-wrap gap-1">
                  {topic.keywords.slice(0, 3).map((kw, i) => (
                    <span
                      key={i}
                      className="px-2 py-0.5 rounded-md bg-slate-950/70 border border-slate-800 text-[10px] text-slate-400"
                    >
                      {kw}
                    </span>
                  ))}
              ...
              <div className="mt-5 pt-3 border-t border-slate-800/80 flex items-center justify-between gap-2">
                ...
                >
                  <option value="not_started" className="bg-slate-900 text-slate-300">
                    Not Started
                  </option>
                  <option value="in_progress" className="bg-slate-900 text-amber-400">
                    In Progress
                  </option>
                ...
                <button
                  onClick={() => onOpenTopic(topic.id)}
                  className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-indigo-600/20 hover:bg-indigo-600 text-indigo-300 hover:text-white border border-indigo-500/30 transition-all flex items-center gap-1 shrink-0"
                >
                  <span>Learn</span>
