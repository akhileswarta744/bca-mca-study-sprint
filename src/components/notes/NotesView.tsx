    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-16">
      ...
      <div className="p-6 md:p-8 rounded-3xl bg-stone-900/80 border border-stone-800 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button
            ...
            className="px-5 py-3 rounded-2xl bg-red-600 hover:bg-red-500 text-white font-bold text-xs shadow-lg shadow-red-600/30 transition-all flex items-center gap-2 self-start md:self-auto shrink-0"
          >
      ...
      {(isCreatingNew || editingNote) && (
        <form
          ...
          className="p-6 rounded-2xl bg-stone-900 border border-red-500/40 shadow-2xl space-y-4 animate-fadeIn"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-stone-950 border border-stone-700 text-white text-sm focus:outline-none focus:border-red-500"
              />
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              className="w-full p-4 rounded-xl bg-stone-950 border border-stone-700 text-white text-sm focus:outline-none focus:border-red-500 font-mono"
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-3">
            <button
              ...
              className="px-4 py-2 rounded-xl text-xs font-semibold text-stone-400 hover:text-white bg-stone-800"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-5 py-2 rounded-xl text-xs font-bold text-white bg-red-600 hover:bg-red-500 shadow-md shadow-red-600/30"
            >
              Save Note
            </button>
          </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-2xl bg-stone-900/80 border border-stone-800 shadow-lg">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-xl bg-stone-950 border border-stone-800 text-xs text-white placeholder-stone-400 focus:outline-none focus:border-red-500"
          />
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0 scrollbar-none text-xs">
          ...
          <select
            ...
            className="bg-stone-950 text-white text-xs font-semibold px-3 py-2 rounded-xl border border-stone-800 focus:outline-none cursor-pointer"
          >
          ...
          return (
            <div
              ...
              className={`p-5 rounded-2xl border transition-all flex flex-col justify-between shadow-lg relative group ${
                note.isPinned
                  ? 'bg-stone-900 border-red-500/40 ring-1 ring-red-500/20 shadow-red-950/20'
            ...
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between gap-2">
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-stone-800 text-red-300">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      <Pin className={`w-3.5 h-3.5 ${note.isPinned ? 'fill-red-400' : ''}`} />
                    </button>
                    <button
                ...
                <div className="text-xs text-stone-300 font-mono whitespace-pre-wrap leading-relaxed line-clamp-6 bg-stone-950/40 p-3 rounded-xl border border-stone-800/80">
                  {note.content}
                </div>
                ...
                {note.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 pt-1">
                    {note.tags.map((tag, tIdx) => (
                      <span
                        key={tIdx}
                        className="px-2 py-0.5 rounded bg-stone-950 text-[10px] text-stone-400"
                      >
                        #{tag}
                      </span>
                    ))}
              ...
              {/* Note Footer: Date */}
              <div className="mt-4 pt-2.5 border-t border-stone-800/80 text-[10px] text-stone-400 flex items-center justify-between">
                <span>Updated: {new Date(note.updatedAt).toLocaleDateString()}</span>
                {note.isPinned && (
                  <span className="text-red-400 font-semibold uppercase">Pinned</span>
                )}
              </div>
            </div>
          );
