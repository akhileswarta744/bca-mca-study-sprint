  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto pb-20">
      ...
      {(statusSaveToast || savedNoteNotification) && (
        <div className="fixed top-5 right-5 z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl bg-emerald-500 text-stone-950 font-bold text-xs shadow-2xl shadow-emerald-500/30 animate-bounce">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 md:p-5 rounded-2xl bg-stone-900/90 border border-stone-800 shadow-xl">
        <div className="flex items-center gap-3">
          <select
            value={selectedTopicId}
            onChange={(e) => setSelectedTopicId(e.target.value)}
            className="bg-stone-950 text-white font-bold text-sm px-3.5 py-2.5 rounded-xl border border-stone-700/80 focus:outline-none focus:border-red-500 max-w-xs md:max-w-md truncate"
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-1 ${
                currentStatus === 'completed'
                  ? 'bg-emerald-500 text-stone-950 font-bold shadow-sm shadow-emerald-500/20'
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl bg-stone-800 hover:bg-stone-700 text-stone-300 hover:text-white border border-stone-700 text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm"
      ...
      {/* Main Topic Header */}
      <div className="p-6 md:p-8 rounded-3xl bg-stone-900/80 border border-stone-800 shadow-xl space-y-3 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-red-500/10 rounded-full blur-3xl pointer-events-none" />

        <div className="relative z-10 flex flex-wrap items-center gap-2">
          <span className="px-2.5 py-1 rounded-md bg-red-500/15 border border-red-500/30 text-red-300 text-xs font-bold uppercase tracking-wider">
            {currentSubject?.name}
          </span>
          <span className="px-2.5 py-1 rounded-md bg-stone-800 text-stone-400 text-xs font-semibold">
            Day {currentTopicMeta.dayNumber}
          </span>
          <span className="px-2.5 py-1 rounded-md bg-stone-800 text-stone-400 text-xs font-semibold capitalize">
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {currentTopicMeta.keywords.map((kw, i) => (
            <span
              key={i}
              className="px-2.5 py-0.5 rounded-full bg-stone-950/80 border border-stone-800 text-xs text-stone-400"
            >
              #{kw}
            </span>
          ))}
      ...
      {content ? (
        <div className="space-y-6">
          ...
          <section className="p-6 rounded-2xl bg-stone-900/80 border border-stone-800 space-y-3 shadow-lg">
            <div className="flex items-center gap-2 text-red-400 font-bold text-xs uppercase tracking-wider">
              <span className="w-5 h-5 rounded-full bg-red-500/20 flex items-center justify-center text-[11px]">
                1
              </span>
              <span>What is it?</span>
            </div>
            <p className="text-base text-stone-200 font-medium leading-relaxed">
              {content.whatIsIt}
            </p>
          </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <section className="p-6 rounded-2xl bg-gradient-to-br from-stone-900 to-red-950/20 border border-red-500/20 space-y-3 shadow-lg">
              ...
              <p className="text-sm text-stone-300 leading-relaxed">{content.simpleExplanation}</p>
            </section>

            {/* SECTION 3: Technical Explanation */}
            <section className="p-6 rounded-2xl bg-gradient-to-br from-stone-900 to-orange-950/20 border border-orange-500/20 space-y-3 shadow-lg">
              <div className="flex items-center gap-2 text-orange-300 font-bold text-xs uppercase tracking-wider">
                <span className="w-5 h-5 rounded-full bg-orange-500/20 flex items-center justify-center text-[11px]">
                  3
                </span>
                <BookOpen className="w-3.5 h-3.5" />
                <span>Technical Explanation</span>
              </div>
              <p className="text-sm text-stone-300 leading-relaxed">{content.technicalExplanation}</p>
            </section>
          </div>

          {/* SECTION 4: Important Definitions */}
          <section className="p-6 rounded-2xl bg-stone-900/80 border border-stone-800 space-y-4 shadow-lg">
            <div className="flex items-center gap-2 text-red-400 font-bold text-xs uppercase tracking-wider">
              <span className="w-5 h-5 rounded-full bg-red-500/20 flex items-center justify-center text-[11px]">
                4
              </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {content.importantDefinitions.map((def, i) => (
                <div
                  key={i}
                  className="p-4 rounded-xl bg-stone-950/60 border border-stone-800/80 space-y-1"
                >
                  <h4 className="text-sm font-bold text-white flex items-center gap-1.5">
                    <span className="text-red-400">•</span> {def.term}
                  </h4>
                  <p className="text-xs text-stone-300 leading-relaxed pl-3.5">{def.definition}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="p-6 rounded-2xl bg-stone-900/80 border border-stone-800 space-y-4 shadow-lg">
            ...
            <ul className="space-y-2.5">
              {content.keyConcepts.map((concept, i) => (
                <li
                  key={i}
                  className="flex items-start gap-3 p-3 rounded-xl bg-stone-950/40 border border-stone-800/80 text-sm text-stone-200"
                >
                  <span className="w-5 h-5 rounded-md bg-red-500/15 text-red-300 font-bold flex items-center justify-center text-xs shrink-0 mt-0.5">
                    {i + 1}
                  </span>
          ...
          <section className="p-6 rounded-2xl bg-stone-900/80 border border-stone-800 space-y-4 shadow-lg">
            <div className="flex items-center justify-between">
              ...
              <span className="px-2 py-0.5 rounded text-[11px] font-mono bg-stone-800 text-stone-300 uppercase">
                {content.example.type}
              </span>
            </div>

            <p className="text-xs text-stone-300 leading-relaxed">{content.example.explanation}</p>

            {/* Code / SQL snippet block */}
            {content.example.code && (
              <div className="relative rounded-xl overflow-hidden border border-stone-800 bg-[#0a0807] text-xs">
                <div className="px-4 py-2 bg-stone-900/90 border-b border-stone-800 flex items-center justify-between text-stone-400 font-mono text-[11px]">
            ...
            {content.example.diagramAscii && (
              <div className="relative rounded-xl overflow-hidden border border-stone-800 bg-[#0a0807] text-xs">
                <div className="px-4 py-2 bg-stone-900/90 border-b border-stone-800 text-stone-400 font-mono text-[11px]">
                  Visual Architecture Representation
                </div>
                <pre className="p-4 overflow-x-auto text-orange-300 font-mono leading-tight whitespace-pre">
                  {content.example.diagramAscii}
                </pre>
              </div>
            )}
            ...
            {content.example.output && (
              <div className="p-3.5 rounded-xl bg-stone-950 border border-stone-800/80 font-mono text-xs">
          ...
          <section className="p-6 rounded-2xl bg-stone-900/80 border border-stone-800 space-y-4 shadow-lg">
            ...
            <div className="space-y-3">
              {content.commonInterviewQuestions.map((iq, i) => (
                <div
                  key={i}
                  className="p-4 rounded-xl bg-stone-950/60 border border-stone-800 space-y-2"
                >
                  <h4 className="text-sm font-bold text-white flex items-start gap-2">
                    <span className="text-red-400 font-mono font-bold shrink-0">Q{i + 1}:</span>
                    <span>{iq.question}</span>
                  </h4>
                  <div className="text-xs text-stone-300 pl-6 leading-relaxed bg-stone-900/60 p-3 rounded-lg border border-stone-800/80">
                    <strong className="text-red-300 block mb-1">Model Answer:</strong>
                    {iq.answer}
                  </div>
                  {iq.tips && (
              ...
              {content.commonMistakes.map((mis, i) => (
                ...
                >
                  <div className="text-xs font-bold text-rose-300 flex items-center gap-2">
                    ...
                    <span className="text-stone-200 font-medium">{mis.mistake}</span>
                  </div>
          ...
          <section className="p-6 rounded-2xl bg-gradient-to-r from-red-950/40 to-stone-900 border border-red-500/30 space-y-4 shadow-lg">
            <div className="flex items-center gap-2 text-red-300 font-bold text-xs uppercase tracking-wider">
              <span className="w-5 h-5 rounded-full bg-red-500/20 flex items-center justify-center text-[11px]">
                9
              </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
              {content.quickRevision.map((point, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2.5 p-3 rounded-xl bg-stone-900/80 border border-stone-800 text-xs text-stone-200"
                >
          ...
          <section className="p-6 rounded-2xl bg-stone-900/80 border border-stone-800 space-y-5 shadow-lg">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-red-400 font-bold text-xs uppercase tracking-wider">
                <span className="w-5 h-5 rounded-full bg-red-500/20 flex items-center justify-center text-[11px]">
                  10
            ...
            <div className="space-y-4">
              {content.practiceQuestions.map((q, qIndex) => {
                ...
                return (
                  <div
                    key={q.id}
                    className="p-5 rounded-xl bg-stone-950/70 border border-stone-800/80 space-y-3"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <h4 className="text-sm font-bold text-white leading-snug">
                        <span className="text-red-400 mr-1.5">Q{qIndex + 1}.</span>
                        {q.question}
                      </h4>
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-stone-800 text-stone-400 shrink-0 uppercase">
                        PRACTICE QUESTION
                      </span>
                    </div>
                    ...
                    <div className="space-y-2 pt-1">
                      {q.options.map((opt, optIdx) => {
                        ...
                        let btnStyle = 'bg-stone-900 border-stone-800 text-stone-300 hover:bg-stone-800/80 hover:border-stone-700';
                        if (isAnswered) {
                          if (isThisCorrect) {
                      ...
                      <div
                        className={`p-3.5 rounded-xl text-xs space-y-1.5 ${
                          isCorrect
                            ...
                            : 'bg-rose-950/20 border border-rose-500/30 text-stone-300'
                        }`}
                      >
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-r from-stone-900 to-red-950/40 border border-stone-800 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              {currentStatus !== 'completed' ? (
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-stone-950 font-bold text-xs shadow-lg shadow-emerald-500/20 transition-all cursor-pointer"
                >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => handleStatusChange('in_progress')}
                    className="px-3.5 py-2 rounded-xl bg-stone-800 hover:bg-stone-700 text-stone-300 hover:text-white border border-stone-700 text-xs font-semibold transition-all"
                  >
                    Reopen Topic
                  </button>
                </div>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-stone-800">
        {prevTopic ? (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-stone-900 hover:bg-stone-800/80 text-stone-300 hover:text-white border border-stone-800 text-xs font-semibold transition-all"
          >
        ...
        {nextTopic ? (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-semibold text-xs shadow-md shadow-red-600/20 transition-all ml-auto"
          >
